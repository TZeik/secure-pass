...
export class ReportService {
  ...
  private static async generateResidentReport(
    ...
    const intervalVisits = allVisits.filter(visit => 
      visit.authorization.date >= start && visit.authorization.date <= end
    );
    ...
    const entryHours = this.calculateMostFrequentHour(
      intervalVisits.filter(v => v.registry?.entry?.date).map(v => v.registry?.entry?.date as Date)
    );
    const exitHours = this.calculateMostFrequentHour(
      intervalVisits.filter(v => v.registry?.exit?.date).map(v => v.registry?.exit?.date as Date)
    );
    ...
    if (guardCounts.size > 0) {
      const [maxGuardId, maxCount] = [...guardCounts.entries()].reduce(
  ...
  /* Reporte de Guardia */
  private static async generateGuardReport(
    guard: IUser,
    start: Date,
    end: Date
  ): Promise<IReport> {
    const allVisits = await Visit.find({
      $or: [
        { "registry.entry.guard": guard._id },
        { "registry.exit.guard": guard._id },
      ],
    }).populate("authorization.resident", "name apartment")
      .populate("registry.entry.guard", "name")
      .populate("registry.exit.guard", "name");
    ...
    const entryVisits = intervalVisits.filter(v => v.registry?.entry?.guard?._id.toString() === guard._id.toString());
    const exitVisits = intervalVisits.filter(v => v.registry?.exit?.guard?._id.toString() === guard._id.toString());
    ...
    if (residentCounts.size > 0) {
      const [maxResidentId, maxCount] = [...residentCounts.entries()].reduce(
        (max, entry) => entry[1] > max[1] ? entry : max
      );
    ...
    return {
      ...
      statistics: {
        totalEntries: allVisits.filter(v => v.registry?.entry?.guard?._id.toString() === guard._id.toString()).length,
        intervalEntries: entryVisits.length,
        totalExits: allVisits.filter(v => v.registry?.exit?.guard?._id.toString() === guard._id.toString()).length,
  ...
  /* Reporte General */
  private static async generateGeneralReport(
    start: Date,
    end: Date
  ): Promise<IReport> {
  ...
  private static calculateMostFrequentHour(dates: Date[]): string {
    ...
    const [mostFrequentHour] = [...hourCounts.entries()].reduce(
      (max, entry) => entry[1] > max[1] ? entry : max
    );
